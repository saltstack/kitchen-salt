...
salt_yum_rpm_key | https://repo.saltstack.com/yum/redhat/7/x86_64/archive/%s/SALTSTACK-GPG-KEY.pub" | The uri to the public key imported using rpm before adding the yum repo. `%s` is replaced with salt_version. For more information check [SaltStack Package Repo](https://repo.saltstack.com/)
salt_yum_repo | https://repo.saltstack.com/yum/redhat/$releasever/$basearch/archive/%s" | The uri to the yum repo. `%s` is replaced with salt_version. For more information check [SaltStack Package Repo](https://repo.saltstack.com/)
salt_yum_repo_key | https://repo.saltstack.com/yum/redhat/$releasever/$basearch/archive/%s/SALTSTACK-GPG-KEY.pub" | The uri to the key for the yum repo. `%s` is replaced with salt_version. For more information check [SaltStack Package Repo](https://repo.saltstack.com/)
salt_yum_repo_latest | https://repo.saltstack.com/yum/redhat/salt-repo-latest-2.el7.noarch.rpm" | The uri to the rpm file for the latest version of salt. Used to install if salt_version is latest. For more information check [SaltStack Package Repo](https://repo.saltstack.com/)
